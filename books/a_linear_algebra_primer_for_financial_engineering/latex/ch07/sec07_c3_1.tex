...
% "X = U^tZ"). Chunks c1, c2 and c4 all instead used a bold CAPITAL subscript
% throughout (\Sigma_{\bfX}, \mu_{\bfX}, \Sigma_{\bfZ}, ...). This chunk's
% files (c3) use the verified lowercase subscript (\bfx, \bfz, \bfy) to match
% the source exactly, which means ch07 is currently inconsistent across
% chunks. Recommend a chapter-wide (book-wide?) mechanical pass replacing
% "_{\bfX}" -> "_{\bfx}", "_{\bfZ}" -> "_{\bfz}", "_{\bfY}" -> "_{\bfy}"
% wherever \bfX/\bfZ/\bfY appears as a SUBSCRIPT on Sigma, mu, or Omega
% (leaving standalone \bfX/\bfZ/\bfY, i.e. the vector symbols themselves,
% untouched). Flagging rather than editing sec07_c1_*/sec07_c2_*/sec07_c4_*,
% which are outside this chunk's ownership.
