...
% Textstyle fractions (as printed) bring this to ~376pt against a 338pt measure,
% so it still needs a light \resizebox -- now ~0.90 (approx 9.8pt type) rather
% than the 0.76 it used with \dfrac. The source keeps all three matrices on one
% line, so scaling is preferable to breaking the product.
\[
\resizebox{\linewidth}{!}{$\displaystyle
A \;=\; \begin{pmatrix} \sqrt{A(1,1)} & \bfzero^t \\[0.6em]
\frac{(A(1,2:n))^t}{\sqrt{A(1,1)}} & I_{n-1} \end{pmatrix}
\begin{pmatrix} 1 & \bfzero^t \\[0.6em]
\bfzero & A_{n-1} - \frac{(A(1,2:n))^tA(1,2:n)}{A(1,1)} \end{pmatrix}
...
\bfzero & A_{n-1} - \frac{(A(1,2:n))^tA(1,2:n)}{A(1,1)} \end{pmatrix} M,
\end{equation}
where $M = \begin{pmatrix} \sqrt{A(1,1)} & \frac{A(1,2:n)}{\sqrt{A(1,1)}} \\[0.6em]
\bfzero & I_{n-1} \end{pmatrix}$ and $\mathbf{0}$ denotes the column vector of size
...
\begin{pmatrix} 1 & \bfzero^t \\ \bfzero & U_{n-1}^t \end{pmatrix}
\;=\; \begin{pmatrix} \sqrt{A(1,1)} & \bfzero^t \\[0.6em]
\frac{(A(1,2:n))^t}{\sqrt{A(1,1)}} & U_{n-1}^t \end{pmatrix};
\end{equation}
\begin{equation}
\begin{pmatrix} 1 & \bfzero^t \\ \bfzero & U_{n-1} \end{pmatrix}
\begin{pmatrix} \sqrt{A(1,1)} & \frac{A(1,2:n)}{\sqrt{A(1,1)}} \\[0.6em]
\bfzero & I_{n-1} \end{pmatrix}
\;=\; \begin{pmatrix} \sqrt{A(1,1)} & \frac{A(1,2:n)}{\sqrt{A(1,1)}} \\[0.6em]
\bfzero & U_{n-1} \end{pmatrix}.
\end{equation}

From (10.138--10.142), we obtain that
% Folio 314: this chain carries no equation tag in the source; typesetting it as
% a numbered align made every later number in the chapter one too high.
\begin{align*}
A &= \begin{pmatrix} \sqrt{A(1,1)} & \bfzero^t \\[0.6em]
\frac{(A(1,2:n))^t}{\sqrt{A(1,1)}} & I_{n-1} \end{pmatrix}
\begin{pmatrix} 1 & \bfzero^t \\ \bfzero & U_{n-1}^t \end{pmatrix}
...
&= U_n^tU_n,
\end{align*}
where $U_n = \begin{pmatrix} \sqrt{A(1,1)} & \frac{A(1,2:n)}{\sqrt{A(1,1)}} \\[0.6em]
\bfzero & U_{n-1} \end{pmatrix}$ is an $n \times n$ upper triangular matrix. Note that the
